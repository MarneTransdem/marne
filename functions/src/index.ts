...
api.post('/api/send-email', async (req, res): Promise<void> => {
  ...
  if (type === 'quote-reminder') {
    ...
    if (!quote) {
      res.status(400).json({ error: "Données du devis manquantes" });
      return;
    }
    ...
    try {
      const buffer = await generatePdfBuffer('devis', quote);
      const base64Attachment = buffer.toString('base64');
      ...
      res.status(500).json({ error: "Échec de l'envoi de la relance devis", details: error.message || error });
      return;
    }
  }
