...
const Services: React.FC = () => {
  ...
  return (
    <div className="bg-white dark:bg-slate-950 transition-colors duration-300">
          ...
          <div className="mb-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {SERVICES.filter(s => ['garde-meuble', 'monte-meuble', 'emballage', 'cartons', 'longue-distance', 'oeuvres-art', 'piano'].includes(s.id)).map((service, idx) => (
                ...
                >
                  <Link to={service.path} className="group bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 hover:border-accent transition-all hover:shadow-2xl flex flex-col h-full">
                    <div className="w-16 h-16 bg-slate-50 dark:bg-slate-800 text-accent rounded-2xl flex items-center justify-center mb-8 group-hover:bg-accent group-hover:text-white transition-colors">
                      <service.icon size={32} />
                    </div>
                    <h3 className="text-2xl font-bold text-brand-900 dark:text-white mb-4 uppercase tracking-tight italic">{service.title}</h3>
                    <p className="text-slate-500 dark:text-slate-400 font-light leading-relaxed mb-10 flex-grow">
                      {service.description}
                    </p>
      ...
      <section className="py-24 bg-slate-50 dark:bg-slate-900/50">
        <div className="container mx-auto px-4 md:px-6 text-center">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-3xl lg:text-4xl font-bold text-brand-900 dark:text-white mb-6">Besoin d'un accompagnement personnalisé ?</h2>
            <p className="text-slate-500 dark:text-slate-400 font-light text-lg mb-12">
            ...
            <Link to="/formules-demenagement" className="inline-flex items-center gap-3 bg-brand-900 dark:bg-slate-900 text-white px-10 py-5 rounded-full font-bold text-lg hover:bg-brand-800 dark:hover:bg-slate-800 shadow-xl transition-all">
               Voir les formules
      ...
      <section className="py-24 dark:bg-slate-950">
        <div className="container mx-auto px-4 md:px-6">
          <div className="bg-accent rounded-[3rem] p-12 lg:p-20 text-white text-center relative overflow-hidden transition-colors duration-300">
             <div className="absolute top-0 left-0 w-32 h-32 bg-white/10 blur-3xl -translate-y-1/2 -translate-x-1/2"></div>
             <h2 className="text-3xl lg:text-5xl font-black mb-8 relative z-10 text-white">Un projet spécifique ?</h2>
             ...
             <div className="flex flex-col sm:flex-row gap-6 justify-center relative z-10">
                <Link to="/demande-de-devis" className="bg-brand-900 dark:bg-slate-950 text-white px-12 py-5 rounded-full font-bold text-lg hover:bg-brand-800 dark:hover:bg-slate-900 transition-all shadow-xl">
                  Demander un devis
                </Link>
                <Link to="/contact" className="bg-white dark:bg-slate-800 text-brand-900 dark:text-white px-12 py-5 rounded-full font-bold text-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-all shadow-xl">
                  Nous contacter
                </Link>
             </div>
          </div>
