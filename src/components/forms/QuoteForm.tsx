...
Cette estimation est indicative et pourra être affinée selon les accès et les caractéristiques de votre projet.`
  ...
  if (isSuccess) {
    return (
      <motion.div 
        ...
        className="bg-white p-10 md:p-16 rounded-3xl shadow-2xl text-center border border-slate-100"
      >
        <div className="w-24 h-24 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto mb-8 shadow-inner">
          <Check size={48} strokeWidth={3} />
        </div>
        <h2 className="text-3xl md:text-4xl font-bold text-brand-900 mb-4">Demande envoyée !</h2>
        ...
        <button 
          ...
          className="bg-brand-900 text-white px-10 py-5 rounded-full font-bold hover:bg-brand-hover transition-all shadow-lg hover:shadow-xl active:scale-95"
        >
          Retour à l'accueil
        </button>
      </motion.div>
  ...
  return (
    <div className="bg-white rounded-3xl shadow-premium border border-slate-100 overflow-hidden">
      {estimate && (
        <div className="bg-brand-900 text-white p-6 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-12 md:space-y-20">
          ...
          <section className="space-y-8 md:space-y-10">
            <div className="border-l-4 border-accent pl-5 md:pl-6 text-left">
              ...
              <p className="text-slate-400 mt-1 md:mt-2 font-light text-sm">Informations essentielles pour notre premier échange.</p>
            </div>
          ...
          <section className="space-y-8 md:space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-brand-900 ml-1">Ville</label>
                <input name="fromCity" value={formData.fromCity} onChange={handleChange} className={`form-input-premium w-full ${errors.fromCity ? 'border-red-500 bg-red-50' : ''}`} placeholder="Paris" />
                {errors.fromCity && <p className="text-red-500 text-xs ml-1 font-medium">{errors.fromCity}</p>}
              </div>
              <div className="space-y-2">
                <label className="text-sm font-bold text-brand-900 ml-1">Code postal</label>
                <input name="fromZip" value={formData.fromZip} onChange={handleChange} className={`form-input-premium w-full ${errors.fromZip ? 'border-red-500 bg-red-50' : ''}`} placeholder="75004" />
                {errors.fromZip && <p className="text-red-500 text-xs ml-1 font-medium">{errors.fromZip}</p>}
              </div>
              <div className="space-y-2">
                <label className="text-sm font-bold text-brand-900 ml-1">Étage</label>
                <input name="fromFloor" value={formData.fromFloor} onChange={handleChange} className="form-input-premium w-full" placeholder="Ex: 3" />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-bold text-brand-900 ml-1">Ascenseur</label>
              ...
              <div className="md:col-span-2 space-y-2">
                <label className="text-sm font-bold text-brand-900 ml-1">Difficultés d’accès éventuelles</label>
          ...
          <section className="space-y-8 md:space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-brand-900 ml-1">Ville</label>
                <input name="toCity" value={formData.toCity} onChange={handleChange} className={`form-input-premium w-full ${errors.toCity ? 'border-red-500 bg-red-50' : ''}`} placeholder="Paris" />
                {errors.toCity && <p className="text-red-500 text-xs ml-1 font-medium">{errors.toCity}</p>}
              </div>
              <div className="space-y-2">
                <label className="text-sm font-bold text-brand-900 ml-1">Code postal</label>
                <input name="toZip" value={formData.toZip} onChange={handleChange} className={`form-input-premium w-full ${errors.toZip ? 'border-red-500 bg-red-50' : ''}`} placeholder="75008" />
          ...
          <section className="space-y-8 md:space-y-10">
            <div className="border-l-4 border-accent pl-5 md:pl-6 text-left">
              ...
              <p className="text-slate-400 mt-1 md:mt-2 font-light text-sm">Précisez les contours logistiques de votre projet.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-brand-900 ml-1">Volume estimé en m³, si connu</label>
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-brand-900 ml-1">Besoin de monte-meuble</label>
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-brand-900 ml-1">Besoin d’emballage</label>
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-brand-900 ml-1">Besoin de garde-meuble</label>
              ...
              <div className="md:col-span-2 space-y-2">
                <label className="text-sm font-bold text-brand-900 ml-1">Message complémentaire</label>
          ...
          <section className="pt-10 border-t border-slate-100 space-y-12">
            <div className="border-l-4 border-accent pl-5 md:pl-6 text-left">
              <h3 className="text-xl md:text-2xl font-bold text-brand-900 tracking-tight">
                E. Consentement
              </h3>
              <p className="text-slate-400 mt-1 md:mt-2 font-light text-sm">Votre accord pour le traitement de votre demande.</p>
            </div>
              ...
              <label className="flex items-start gap-4 cursor-pointer group">
                <div className="mt-1">
                  <input 
                    ...
                    onChange={handleChange}
                    className="w-6 h-6 rounded border-slate-300 text-accent focus:ring-accent transition-all cursor-pointer"
                  />
                </div>
